...
def main(cfg: VideoConfig):
    ...
    print(f"ckpt_path: {ckpt_path}") # lihao
    assert os.path.exists(ckpt_path)

    renderer = Renderer.init_from_checkpoint(
        ckpt_path,
        device,
        use_2dgs = cfg.use_2dgs,# lihao
        work_dir=cfg.work_dir,
        port=None,
    )
    print(f"[lihao-mof]: {train_dataset.num_frames=} | {renderer.num_frames=}")
    ...
    # print("------lihao-------w2cs.shape,ts.shape================",w2cs.shape,ts.shape)
    ...
    with torch.inference_mode():
        render_outs = renderer.model.render(
            ...
            img_wh,
            target_ts=ts, 
            # target_w2cs = w2cs, # lihao-mof,add,render_tracks.py时不可用
            return_color=True,
            fg_only=True,
    ...
    # gt_mask = train_dataset.masks[0][::grid, ::grid].to(device)  # lihao,踩过的坑, 由于作者将子类get_mask方法定义成了.get_masks导致，但调用的get_mask。
