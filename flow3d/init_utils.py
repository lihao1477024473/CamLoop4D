...
#     print(f"[num_gaussians, 3]: {tracks_3d.xyz.shape=} | {means_cano.shape=}") # lihao
#     # exit(-1)
...
def init_motion_params_with_procrustes(
    ...
    dists2centers = torch.norm(means_cano[:, None] - sampled_centers, dim=-1)
    motion_coefs = 10 * torch.exp(-dists2centers)
    print(f"motion_coefs = 10 * torch.exp(-dists2centers)[lihao-mof]: {motion_coefs=} {motion_coefs.shape=}")
    ...
    init_rots = id_rot.reshape(1, 1, rot_dim).repeat(num_bases, num_frames, 1) # ori
    init_ts = torch.zeros(num_bases, num_frames, 3, device=device)
    # # [lihao-mof]
    # num_gaussians = tracks_3d.xyz.shape[0]
    # motion_coefs =  moflh.init_motion_coefs(num_gaussians,num_frames,3) # [N,F,B]
    # init_rots = moflh.rots_6d # [B,6]
    # init_ts = moflh.transls_3d # [b,3]
    # guru.info(f"{motion_coefs.shape=} {init_rots.shape=} {init_ts.shape=}")
    # # [lihao-mof]
    errs_before = np.full((num_bases, num_frames), -1.0)
    errs_after = np.full((num_bases, num_frames), -1.0)
    ...
    # [lihao-mof]
    num_gaussians = tracks_3d.xyz.shape[0]
    # motion_coefs =  moflh.init_motion_coefs(num_gaussians,num_frames,moflh.num_base) # [N,F,B]
    # init_rots = moflh.rots_6d # [B,6]
    # init_ts = moflh.transls_3d # [b,3]
    # guru.info(f"{motion_coefs.shape=} {init_rots.shape=} {init_ts.shape=}")

    # [1] coef1*base1 = coef2*base2
    # 方式1:2507
    ...
    # [lihao-mof]
    # bases = MotionBases(init_rots, init_ts) 
    # bases = MotionBases(init_rots2, init_ts2) # [lihao-mof]
    bases = MotionBases(init_rots2, init_ts2,num_frames=num_frames) # [lihao-mof]

    # [lihao-mof]
    # bases.num_frames = num_frames
    # moflh.NUM_FRAME = num_frames
    guru.info(f"*********************** {bases.num_frames=} ***********************")
    # exit(-1)
    # [lihao-mof]
    return bases, motion_coefs, tracks_3d
...
def run_initial_optim(
    ...
    for i in pbar:
        ...
        # print(f"[lihao-mof]*******************:ts_neighbors={ts_neighbors.shape} ,coefs={coefs.shape}*******************")
        # print(f"[lihao-mof]*******************:transfms_nbs={transfms_nbs.shape}*******************")
        # print(f"[lihao-mof]*******************:means={fg.params["means"].shape},means_pad={ F.pad(fg.params["means"], (0, 1), value=1.0).shape} *******************")
        # print(f"[lihao-mof]*******************:means_nbs={means_nbs.shape} w2cs={w2cs.shape} *******************")
        means_nbs = means_nbs.reshape(means_nbs.shape[0], 3, -1, 3)  # [G, 3, n, 3]
        # print(f"[lihao-mof]*******************:means_nbs={means_nbs.shape} w2cs={w2cs.shape} *******************-1")
        # exit(-1)
...
def sample_initial_bases_centers(
    ...
    device = means_canonical.device # lihao

    num_bases = labels.max().item() + 1
    sampled_centers = torch.stack(
        [
            # means_canonical[torch.tensor(labels == i)].median(dim=0).values # 原来
            means_canonical[torch.tensor(labels == i).to(device)].median(dim=0).values # lihao
            for i in range(num_bases)
        ]
    )[None]
    print("number of {} clusters: ".format(mode), num_bases)
    # return sampled_centers, num_bases, torch.tensor(labels) # 原来
    return sampled_centers, num_bases, torch.tensor(labels).to(device) # lihao
