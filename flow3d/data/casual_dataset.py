...
class CustomDataConfig:
    seq_name: str  # lihao === https://github.com/vye16/shape-of-motion/blob/v0.0.1/flow3d/data/casual_dataset.py
    data_dir: str
    start: int = 0
    end: int = -1
    res: str = ""
    ...
    num_targets_per_frame: int = 4
    load_from_cache: bool = False
    skip_load_imgs: bool = False # lihao
...
class CasualDataset(BaseDataset):
    def __init__(
        ...
        **_,
        ...
        # self.seq_name = "garden_flythrough_seva_16"  # lihao ===
        self.seq_name = seq_name # lihao ===
        ...
        self.img_dir = f"{data_dir}/{image_type}/{res}/{self.seq_name}" # lihao ===
        ...
        self.depth_dir = f"{data_dir}/{depth_type}/{res}/{self.seq_name}" # lihao ===
        self.mask_dir = f"{data_dir}/{mask_type}/{res}/{self.seq_name}" # lihao ===
        self.tracks_dir = f"{data_dir}/{track_2d_type}/{res}/{self.seq_name}" # lihao ===
        self.cache_dir = f"{data_dir}/flow3d_preprocessed/{res}/{self.seq_name}" # lihao ===
        ...
        if camera_type == "droid_recon":
            ...
            w2cs, Ks, tstamps = load_cameras(
                ...
                f"{data_dir}/{camera_type}/{self.seq_name}.npy", H, W # lihao ===
            )
    ...
    def load_image(self, index) -> torch.Tensor:
        ...
        # print(f"[load_image]========================================== path======================:{path} {self.img_dir} {self.frame_names} {index} {self.img_ext}")
    ...
    def load_mask(self, index) -> torch.Tensor:
        ...
        # print(f"[load_mask]------------------------ path-------------------------:{path} {self.frame_names[index]}")
    ...
    def get_tracks_3d(
        ...
        # )
        # print(f"[get_tracks_3d]-before: {tracks_3d.shape=}, {visibles.shape=}, {invisibles.shape=}, {confidences.shape=}, {colors.shape=}") # lihao-mof

        # tracks_all_queries = moflh.uniform_sample(tracks_all_queries,1.0)  # lihao-mof
        # [lihao-mof]
        tracks_3d, colors, visibles, invisibles, confidences = map(
            partial(torch.cat, dim=0), zip(*tracks_all_queries)
        )
        print(f"[get_tracks_3d: sample=0.5] {tracks_3d.shape=}, {visibles.shape=}, {invisibles.shape=}, {confidences.shape=}, {colors.shape=}")
    ...
    def get_bkgd_points(
        ...
        **kwargs,
        ...
        print("bg_geometry: ",bg_geometry) # lihao
